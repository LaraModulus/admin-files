<div class="panel panel-default" data-ng-controller="filesController">
    <div class="panel-heading">
        ...
        <div class="btn-group pull-left" role="group" aria-label="Menu">
            <label for="file_upload" class="btn btn-sm btn-default btn-file" data-ng-class="{'disabled': !selected_directory.id}" data-ng-disabled="!selected_directory.id">
                <i class="fa fa-upload"></i> Upload <input type="file" name="file" multiple id="file_upload" style="display: none;">
            </label>
            <button type="button" class="btn btn-sm btn-default" data-ng-class="{'disabled': Files.selected.length!==1}" data-ng-disabled="Files.selected.length!==1" data-ng-click="downloadSelectedFile()"><i class="fa fa-download"></i> Download</button>
            <button type="button" class="btn btn-sm btn-default" data-ng-class="{'disabled': Files.selected.length!==1}" data-ng-disabled="Files.selected.length!==1"><i class="fa fa-pencil-square-o"></i> Edit</button>
        </div>
        <div class="col-xs-5">
            <input type="search" class="form-control input-sm" placeholder="Filter" name="filter" id="files-filter">
        </div>
        <div class="btn-group pull-right" role="group">
            <button type="button" class="btn btn-sm btn-primary" data-ng-click="sync()"><i class="fa fa-refresh"></i></button>
        ...
        <div class="panel panel-default col-sm-9">
            <div class="panel-body">
                <div class="panel" data-ng-if="files_for_upload.length">
                    <div ng-repeat="file in files_for_upload" style="margin-top: 20px;border-bottom-color: antiquewhite;border-bottom-style: double;">
                        <div><span>@{{file.name}}</span><div style="float:right;"><span>@{{file.humanSize}}</span><a data-ng-click="removeFile(file)" title="Remove from list to be uploaded"><i class="icon-remove"></i></a></div>
                        </div>
                        <progress style="width:100%;" value="@{{file.loaded}}" max="@{{file.size}}"></progress>
                    </div>
                    <div>
                        <button type="button" class="btn btn-primary" data-ng-click="startUpload()">Start upload</button>
                ...
                <ul class="list-inline files-list" data-ng-if="files.length && !files_loading">
                    <li class="item" data-ng-class="{selected: user_files.selected_ids.indexOf(file.id)!==-1}" data-ng-repeat="file in files track by $index" data-ng-click="selectFile($index, $event)">
                        ...
                        <div class="text-center image-block">
                            <div class="editor-block text-right">
                                ...
                                <button type="button" class="btn btn-xs btn-danger" data-ng-click="deleteFile($index)"><i class="fa fa-trash"></i></button>
                            </div>
